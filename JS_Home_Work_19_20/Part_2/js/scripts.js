...
//1 - array of skills, uniq, sorted 
var i = 0;
var skillsRes = [];
_.times(men.length-1, function(){
	skillsRes = _.union(men[i].skills, men[1 + i++].skills, skillsRes);
});
skillsRes = _.sortBy(skillsRes);
console.log(skillsRes);

//2 - names, sorted for friend count
var namesArray = [];
i = 0;
_.times(men.length, function(){
	var name = men[i].name;
	var countFriends = men[i].friends.length;
	var elem = {
		'name': name,
...
namesArray = _.sortBy(namesArray, 'countFriends');
var namesRes = [];
i = 0;
_.times(namesArray.length, function(){
	namesRes.push(namesArray[i++].name);
});
...
_.times(men.length, function(){
	var name = men[i++].name;
	friendsRes.push(name);	
});
friendsRes = _.union(friendsRes);
friendsRes = _.sortBy(friendsRes);
